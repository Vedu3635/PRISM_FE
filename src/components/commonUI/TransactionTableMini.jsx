        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-border/50">
              {columns.includes('label') && <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Log Item</th>}
              {columns.includes('date') && <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Date</th>}
              {columns.includes('paidBy') && <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground">Payer</th>}
              {columns.includes('amount') && <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">Amount</th>}
              <th className="pb-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/30">
            {transactions.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-12 text-center text-sm text-muted-foreground">No records found</td>
              </tr>
            ) : (
